export const API_URL = 'http://localhost:5001/v1';
export const API_URL_PROFILE_PHOTO = 'http://localhost:5001/static/personProfilePhoto';
export const API_URL_DOCUMENTO_ADMISION = 'http://localhost:5001/static/documentoAdmision';
export const API_URL_PAGO_ARCHIVO = 'http://192.168.0.41:5001/static/files_pago';
export const API_URL_FILES_TEXTOS = 'http://0.0.0.0:5001/static/files_textos'

export const environment = {
    production: false,
    API_URL: 'http://localhost:5001/v1',
    API_URL_PROFILE_PHOTO: 'http://0.0.0.0:5001/static/personProfilePhoto',
    API_URL_DOCUMENTO_ADMISION: 'http://0.0.0.0:5001/static/documentoAdmision',
    API_URL_PAGO_ARCHIVO: 'http://0.0.0.0:5001/static/files_pago',
    API_URL_FILES_TEXTOS: 'http://0.0.0.0:5001/static/files_textos'
};
